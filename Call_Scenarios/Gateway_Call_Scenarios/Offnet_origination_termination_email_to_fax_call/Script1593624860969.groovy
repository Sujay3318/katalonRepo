...
/*Scenario :Offnet origination termination send Fax Call from one user to another user
 * Account1-UserAgent1
 * Account2-UserAgent2
 * UserAgent1 will send email to UserAgent2(using faxnumber)
 */
...
for (int userDetailsListIndex = 0; userDetailsListIndex < listOfUserDetailsMaps.size(); userDetailsListIndex++) {
    ...
    //store fax_number for future use
 ...
 * using fax_number and account_realm
 */
def receiverMail_1 = (GlobalVariable.user2_creation_details.get('fax_number') + '@') + GlobalVariable.gateway_call_account_details1.get(
    'account_realm')
