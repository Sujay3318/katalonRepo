...
for (int userDetailsListIndex = 0; userDetailsListIndex < listOfUserDetailsMaps.size(); userDetailsListIndex++) {
    ...
    println(CustomKeywords.'kazoo_operation_Utility.ApiCommonOperations.parseResponseData'(res))
 ...
 *caller = user2
 *callee = user1
 *user2 calls to user1 (has DND feature enable)
 */
 ...
 *caller = user1
 *callee = user2
 *user1 calls to user2 (has DND feature enable)
 */
DriverFactory.changeWebDriver(GlobalVariable.user1_creation_details.get('window'))
