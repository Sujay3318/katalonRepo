...
for (int userDetailsListIndex = 0; userDetailsListIndex < listOfUserDetailsMaps.size(); userDetailsListIndex++) {
    def res = CustomKeywords.'cdr_Utility.API_CDR_setup_keyword.enable_DND_feature'(GlobalVariable.sub_account_details.get(
    ...
    def vm_feature_response = CustomKeywords.'cdr_Utility.API_CDR_setup_keyword.enable_VMbox_feature'(GlobalVariable.sub_account_details.get(
            'account_id'), listOfUserDetailsMaps.get(userDetailsListIndex).get('user_id'), listOfUserDetailsMaps.get(userDetailsListIndex).get(
            'callflow_id'), vmbox_id)

    println(CustomKeywords.'kazoo_operation_Utility.ApiCommonOperations.parseResponseData'(vm_feature_response))

    WS.verifyResponseStatusCode(vm_feature_response, 200)
}
