...
/*
 * Scenario: Offnet origination termaination,call forward with forwarder caller Id
 * account1 user1=caller
 * account2 user2=callee (call forward feature is enabled)
 * account3 user3 (third party)
 * user1 calls to user2
 * call is fowarded to user3
 */
/*Enable call forward feature of user2
 *keep forwarder caller_ID
 */
WebUI.callTestCase(findTestCase('CDR/Setup/Gateway_Call_Setup/GenericAccountUserDeviceSetup'), [('accountNumber') : 3], 
    FailureHandling.STOP_ON_FAILURE)

def forwardResponse = CustomKeywords.'cdr_Utility.API_CDR_setup_keyword.enable_call_forwarding_feature'(GlobalVariable.gateway_call_account_details2.get(
        'account_id'), GlobalVariable.user2_creation_details.get('user_id'), GlobalVariable.user3_creation_details.get('phone_number'), 
    false)

println(CustomKeywords.'kazoo_operation_Utility.ApiCommonOperations.parseResponseData'(forwardResponse))

WS.verifyResponseStatusCode(forwardResponse, 200)
