...
/* Scenario: Offnet origination termaination,call transfer caller initiate transfer
* account1 user1=caller
* account2 user2=callee
* account3 user3 (third party)
* user1 calls to user2
* user2 receive the call
* user1 will transfer call to user3
*/
//login to webrtc web app
...
for (int i = 0; i < 2; i++) {
    ...
    //user3 hangup the call
    CustomKeywords.'webRTC_Utility.WEBRTC.hang_up_the_call'()

    WebUI.delay(10)
}

GlobalVariable.user1_creation_details.get('window').quit()

GlobalVariable.user2_creation_details.get('window').quit()

GlobalVariable.user3_creation_details.get('window').quit()
